...
build() {
	...
	./configure --prefix=/data/usr --enable-posix-api
	make
}
...
package() {
	...
	install -dm755 "$pkgdir"/data/usr/share/doc
	cp -r doc "$pkgdir"/data/usr/share/doc/$pkgname
	for licfile in COPYING INSTALL; do
		install -Dm644 $licfile "$pkgdir"/data/usr/share/licenses/$pkgname/$licfile
	done
}
